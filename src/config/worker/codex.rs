...
use crate::core::proc::{Proc, Recv, Stream};
use crate::config::base::consts::{TOOL, VERSION};
...
impl Codex {
    ...
    fn open ( &mut self, cwd: &StdPath, pid_file: Option<&StdPath> ) -> AppResult<Stream> {
        ...
        let id = self.tick();
        let init = json!({ "jsonrpc": "2.0", "id": id, "method": "initialize", "params": { "protocolVersion": "2024-11-05", "capabilities": {}, "clientInfo": { "name": TOOL, "version": VERSION } } });
